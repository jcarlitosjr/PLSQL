create or replace PROCEDURE CIT_ORDER_PROCEDURE_NEW AS
...
BEGIN
  ...
  FOR ORDER_VO IN (
      select distinct o.ORDER_ID, o.STATE, o.SUBMITTED_DATE, o.PROFILE_ID
      from DCSPP_ORDER o
             inner join DCSPP_PAY_GROUP pg on (pg.ORDER_REF = o.ORDER_ID)
             inner join DCSPP_ORDER_ITEM_STG stg on (stg.order_id = o.order_id)
      where o.STATE = 'SUBMITTED' and 'SETTLED' = pg.STATE
 ) LOOP

    select STATE INTO STATE_PAY from DCSPP_PAY_GROUP where ORDER_REF = ORDER_VO.ORDER_ID;
    IF ( 'SUBMITTED' = ORDER_VO.STATE AND 'SETTLED' = STATE_PAY  AND ORDER_VO.SUBMITTED_DATE >  ADD_MONTHS (sysdate, -12) )THEN
        BEGIN
            ...
            AS
             (
              ...
              WHERE PG.STATE = 'SETTLED' and O.STATE  in ('SUBMITTED', 'NO_PENDING_ACTION')
                and O.SUBMITTED_DATE >  ADD_MONTHS (sysdate, -12) AND O.PROFILE_ID = ORDER_VO.PROFILE_ID
              )
              select  cnd.CN_ID as CN_ID , prof.CF_ID as CF_ID, oep.name AS OFFER, cnd.SUBMITTED_DATE AS ORDER_DATE
              ...
              BEGIN
                SELECT DISTINCT MAX(ID_KEY) INTO V_SEQ_REFIL
              ...
              BEGIN
                SELECT CN_ID INTO CN_ID_VAR FROM CIT_OPPORTUNITIES WHERE CF_ID = I.CF_ID;
              EXCEPTION
                WHEN NO_DATA_FOUND THEN
                  CN_ID_VAR := NULL;
              END;
              ...
              BEGIN
                  ...
                  IF CN_ID_VAR IS NOT NULL  THEN
                    DELETE FROM CIT_OPPORTUNITIES WHERE CF_ID = I.CF_ID;
                  END IF;
